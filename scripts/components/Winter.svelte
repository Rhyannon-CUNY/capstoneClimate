  ...
  <div class="background-wrapper">
    ...
    <div class="winter-gif-background">
      ...
      <div class="text-wrapper">
        {#if freezes === 'yes'}
          {#if dataRecord.freezingPast !== null && dataRecord.freezingPast > 0}
            <p>
              In the past, there were <span class="data-value"
                >{dataRecord.freezingPast} day{dataRecord.freezingPast !== 1 ? 's' : ''}</span
              > below freezing.
            </p>
          {/if}
          {#if dataRecord.freezingNow !== null && dataRecord.freezingNow > 0}
            <p>
              Because winters are getting warmer, only <span class="data-value"
                >{dataRecord.freezingNow}</span> {dataRecord.freezingNow === 1 ? 'is' : 'are'} expected to be that cold now.
            </p>
          {/if}
